...
import { advanceBlockTo } from "./utilities/index.cts"
...
describe("MasterChef", function () {
  ...
  beforeEach(async function () {
    ...
    this.sushi = await this.ZSwapToken.deploy(this.signers[0].address)
    await this.sushi.deployed()
  })

  it("should set correct state variables", async function () {
    this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "1000", "0", "1000", this.signers[0].address)
    ...
    console.log('sushi : ', sushi, ' , address: ', this.sushi.address)
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should allow emergency withdraw", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "100", "100", "1000",this.signers[0].address)
      await this.chef.deployed()
    ...
    it("should not distribute SUSHIs if no one deposit", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "100", "200", "1000", this.signers[0].address)
      await this.chef.deployed()
      await this.sushi.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
      await this.lp.connect(this.bob).approve(this.chef.address, "1000")
    ...
    it("should distribute SUSHIs properly for each staker", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "100", "300", "1000", this.signers[0].address)
      await this.chef.deployed()
      await this.sushi.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
    ...
    it("should give proper SUSHIs allocation to each pool", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "100", "400", "1000", this.signers[0].address)
      await this.sushi.transferOwnership(this.chef.address)
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "100", "500", "600", this.signers[0].address)
      await this.sushi.transferOwnership(this.chef.address)
      await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
